...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        ...
        for activity in orm['activity_feed.Activity'].objects.iterator():
            new_user_id = mapper[activity.user_id]

            activity.user_id = new_user_id
            activity.save(update_fields=['user'])

        for activityfeed in orm['activity_feed.ActivityFeed'].objects.iterator():
            new_owner_id = mapper[activityfeed.owner_id]
            new_user_id = mapper[activityfeed.user_id]

            activityfeed.owner_id = new_owner_id
            activityfeed.user_id = new_user_id
            activityfeed.save(update_fields=['owner', 'user'])
