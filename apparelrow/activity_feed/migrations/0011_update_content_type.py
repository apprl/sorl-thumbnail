...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        ...
        for activity in orm['activity_feed.Activity'].objects.iterator():
            if activity.content_type_id == cur_ctype_id:
                if int(activity.object_id) not in mapper:
                    activity.delete()
                    continue

                new_user_id = mapper[activity.object_id]

                activity.object_id = new_user_id
                activity.content_type_id = new_ctype_id
                activity.save(update_fields=['object_id', 'content_type_id'])

        for activityfeed in orm['activity_feed.ActivityFeed'].objects.iterator():
            if activityfeed.content_type_id == cur_ctype_id:
                if int(activityfeed.object_id) not in mapper:
                    activityfeed.delete()
                    continue

                new_user_id = mapper[activityfeed.object_id]

                activityfeed.object_id = new_user_id
                activityfeed.content_type_id = new_ctype_id
                activityfeed.save(update_fields=['object_id', 'content_type_id'])
    ...
    complete_apps = ['profile', 'activity_feed']
    symmetrical = True
