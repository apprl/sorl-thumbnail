...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        for user in orm['profile.User'].objects.iterator():
            if not user.is_brand:
                try:
                    user.facebook_user_id = int(user.username)
                    user.save()
                except ValueError:
                    pass

    def backwards(self, orm):
        orm['profile.User'].objects.update(facebook_user_id=None)
