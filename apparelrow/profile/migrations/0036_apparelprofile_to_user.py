...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        for profile in orm['profile.ApparelProfile'].objects.iterator():
            ...
            user.blog_url = profile.blog_url

            user.is_brand = profile.is_brand
            user.brand = profile.brand

            user.login_flow = profile.login_flow

            user.newsletter = profile.newsletter

            user.discount_notification = profile.discount_notification

            user.fb_share_like_product = profile.fb_share_like_product
            user.fb_share_like_look = profile.fb_share_like_look
            user.fb_share_follow_profile = profile.fb_share_follow_profile
            user.fb_share_create_look = profile.fb_share_create_look

            user.facebook_access_token = profile.facebook_access_token
            user.facebook_access_token_expire = profile.facebook_access_token_expire

            user.comment_product_wardrobe = profile.comment_product_wardrobe
            user.comment_product_comment = profile.comment_product_comment
            user.comment_look_created = profile.comment_look_created
            user.comment_look_comment = profile.comment_look_comment
            user.like_look_created = profile.like_look_created
            user.follow_user = profile.follow_user
            user.facebook_friends = profile.facebook_friends

            user.followers_count = profile.followers_count
            user.popularity = profile.popularity
