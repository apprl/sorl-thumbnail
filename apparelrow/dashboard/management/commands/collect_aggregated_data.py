        ...
        for row in earnings:
            ...
            if row.from_product:
                product_instance, product_created = get_model('dashboard', 'AggregatedData').objects.\
                    get_or_create(user_id=user_id, date=query_date.date(), type='aggregated_from_product', aggregated_from_id=row.from_product.id)
                if product_created:
            ...
            if row.user_earning_type in ('publisher_network_tribute', 'publisher_network_click_tribute'):
                ...
                if row.user_earning_type == 'publisher_network_tribute':
                    ...
                    if row.from_product:
                        product_instance, product_created = get_model('dashboard', 'AggregatedData').objects.\
                    get_or_create(user_id=user_id, date=start_date.date(), type='aggregated_from_product',
