...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        ...
        orm['dashboard.Payment'].objects.filter(paid=True).update(currency='SEK')

        for sale in orm['dashboard.Sale'].objects.iterator():
            if sale.original_currency == 'EUR':
                sale.converted_amount = sale.original_amount
                sale.converted_commission = sale.original_commission
                sale.amount = sale.original_amount
                sale.currency = 'EUR'
                sale.commission = sale.commission / sale.exchange_rate
                sale.exchange_rate = 1
                sale.save()
            else:
                exchange_rate = currency_exchange('EUR', sale.original_currency)
