...
def generate_weekly_mail(request):
    ...
    for product in week_products + base_products:
        if product.pk not in used_products:
            ...
            if product.default_vendor.discount_price:
                product_price = u'<span class="original">%.0f %s</span> <span class="discount">%.0f %s</span>' % (product.default_vendor.price, product.default_vendor.currency, product.default_vendor.discount_price, product.default_vendor.currency)
    ...
    for object_id in Follow.objects.values_list('object_id', flat=True).annotate(count=Count('id')).order_by('-count')[:4]:
        ...
        members.append({
            ...
            'image': ''.join(['http://', Site.objects.get_current().domain, profile.avatar_medium]),
