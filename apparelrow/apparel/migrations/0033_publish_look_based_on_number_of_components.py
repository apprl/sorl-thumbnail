...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        look_ids = []
        for look in orm['apparel.Look'].objects.all():
            component_count = orm['apparel.LookComponent'].objects.filter(component_of=look.component, look=look).count()
            if (look.component == 'P' and look.image) or component_count > 0:
                look_ids.append(look.id)

        orm['apparel.Look'].objects.filter(id__in=look_ids).update(published=True)

    def backwards(self, orm):
        orm['apparel.Look'].objects.update(published=False)
