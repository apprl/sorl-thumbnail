...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        for look in orm['apparel.Look'].objects.all():
            if look.component == 'P' and look.image:
                buf = StringIO(look.image.read())
                image = Image.open(buf)

                width, height = image.size

                look.image_width = width
                look.image_height = height
                look.save(update_fields=['image_width', 'image_height'])
